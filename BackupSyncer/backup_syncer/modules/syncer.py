...
from backup_syncer.modules import setup_file_utils
...
class Syncer:
    ...
    def sync_file(self, src_fp: str, src_fn: str, dst_dir_path: str) -> None:
        dst_dir = os.listdir(dst_dir_path)
        dst_fn = src_fn
        if src_fn in dst_dir:
            dst_fp = os.path.join(dst_dir_path, dst_fn)
            ...
            to_create_item = sync_attribute_create.SyncAttributeCreate(
                original_file_path=src_fp,
                backup_file_path=os.path.join(dst_dir_path, src_fn),
            )
    ...
    def sync_directory(self, src_dir_path: str, dst_dir_path: str) -> None:
        self.search_trash(src_dir_path, dst_dir_path)

        dst_directories = os.listdir(dst_dir_path)
        for src_dir in os.listdir(src_dir_path):
            if os.path.isdir(os.path.join(src_dir_path, src_dir)):
                if src_dir not in dst_directories:
                    to_create_item = sync_attribute_create.SyncAttributeCreate(
                        original_file_path=os.path.join(src_dir_path, src_dir),
                        backup_file_path=os.path.join(dst_dir_path, src_dir),
                    )
                    self.files_to_create.append(to_create_item)

                else:
                    sub_src_dir = os.path.join(src_dir_path, src_dir)
                    sub_dst_dir = os.path.join(dst_dir_path, src_dir)
                    self.sync_directory(sub_src_dir, sub_dst_dir)
            else:
                src_fp = os.path.join(src_dir_path, src_dir)
                self.sync_file(src_fp, src_dir, dst_dir_path)
    ...
    def use_setup(self) -> None:
        for line in self.setup_directories:
            self.sync_directory(src_dir_path=line["src"], dst_dir_path=line["dst"])
